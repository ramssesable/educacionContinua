<?php  
	...
	$search = mysql_query("SELECT email FROM user WHERE email='$email' ");
	...
	if ($row=mysql_fetch_assoc($search)) {
		...
		$update = mysql_query("UPDATE user SET password='$pwd' WHERE email='$email'");
		...
		$cuerpo=$cuerpo."desde aqui puede entrar <a href='http://www.facultaddecienciasadministrativas.com/odontologia/admin/index.php'>Iniciar Secion</a> para que cambie su Password<br>";
